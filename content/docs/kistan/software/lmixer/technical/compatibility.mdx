...
The [MQTT topics](./mqtt_topics) `light_mixer/code/+` were removed and replaced
...
`light_mixer/code/run` [tries to translate](./mqtt_topics#light-mixer-code-run)
the code to support the move to `Scripts.`. The [start](./mqtt_topics#light-mixer-code-startscript)
and [stop](./mqtt_topics#light-mixer-code-stopScript) topics exists as a bridge;
