...
**_Example:_** This computes `(Base*Dim)+(Addition*AdditionDim)`

<LayerTreeExplaination explanationName="layeringExplanation" />
...
<CalloutExt type="info" title="tip">
  The editor can save without running a script by pressing CTRL+S. Useful for a
  looping script that loads the change on the next iteration.
</CalloutExt>
