...
When publishing, simply publish to the [addons topic](./../technical/mqtt_topics#light-mixer-addons).

<CalloutExt type="warn">
  Make sure to have the **retain** flag set when publishing to make your addons
  persistent.
</CalloutExt>
## Splitting up across files

It is way easier to manage a project when split up
across files. By using the [DEPENDENCY Macro](./../macros#dependency)
...
<CalloutExt type="warn">
  Cyclic dependency loops lead to weird initialization orders, and should be
  avoided if possible.
</CalloutExt>

<CalloutExt type="details" title="Technical details">
  `Active Addons` is internally treated as an addon, which is why --#DEPENDENCY
  is used there.
</CalloutExt>
