...
`light_mixer/code/run` [tries to translate](./mqtt-topics#light_mixercoderun)
the code to support the move to `Scripts.`. The [start](./mqtt-topics#light_mixercodestartscript)
and [stop](./mqtt-topics#light_mixercodestopscript) topics exists as a bridge;
