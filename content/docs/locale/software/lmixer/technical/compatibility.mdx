...
The [MQTT topics](./mqtt-topics) `light_mixer/code/+` were removed and replaced
...
`light_mixer/code/run` [tries to translate](./mqtt-topics#light-mixer-code-run)
the code to support the move to `Scripts.`. The [start](./mqtt-topics#light-mixer-code-startscript)
and [stop](./mqtt-topics#light-mixer-code-stopScript) topics exists as a bridge;
